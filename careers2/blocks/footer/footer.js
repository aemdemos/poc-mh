 ...
 * recreate the links when they are missing. Also cleans up any
 * raw :icon-name: text links from the markdown source.
 */
function ensureSocialIcons(footer) {
  ...
  // Remove raw :icon-name: text links (e.g. ":instagram:", ":youtube:")
  const iconTextLinks = cell.querySelectorAll('a');
  iconTextLinks.forEach((a) => {
    if (/^:[\w-]+:$/.test(a.textContent.trim())) {
      a.remove();
    }
  });

  // Only add icon links if none exist yet
